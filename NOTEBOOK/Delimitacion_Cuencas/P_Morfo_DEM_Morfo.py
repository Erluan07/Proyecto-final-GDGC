...
Drenajes_UnSplit_shp = Workspace + "Drenajes_UnSplit.shp"
arcpy.management.UnsplitLine(in_features=Drenajes_Prev_shp, out_feature_class=Drenajes_UnSplit_shp, dissolve_field=[], statistics_fields=[])
...
for fid in range(num_cuencas):
    ...
    arcpy.AddMessage("Procesando cuenca {} de {}".format(fid, num_cuencas))
...
#=====================================================================================
        # Añadir cálculo de morfometría aquí
        arcpy.AddMessage("Calculando morfometría para la cuenca {}".format(ValueP))

        # Definir variables locales para morfometría
        Fill_raster = salida_Fill_Clip
        FlowDir = Workspace + "FlowDir_{}.tif".format(ValueP)
        FlowAcc = Workspace + "FlowAcc_{}.tif".format(ValueP)
        Acuenca = Workspace + "Acuenca_{}.tif".format(ValueP)
        SlopeGrados = Workspace + "SlopeGrados_{}.tif".format(ValueP)
        Slope_Reclass = Workspace + "Slope_Reclass_{}.tif".format(ValueP)
        Aspect = Workspace + "Aspect_{}.tif".format(ValueP)
        Slope_Radians = Workspace + "Slope_Radians_{}.tif".format(ValueP)
        Aspect_Radians = Workspace + "Aspect_Radians_{}.tif".format(ValueP)
        VarianzaEsferica = Workspace + "VarianzaEsferica_{}.tif".format(ValueP)
        ACuenca_Reclass = Workspace + "ACuenca_Reclass_{}.tif".format(ValueP)
        VarianzaEsferReclass = Workspace + "VarianzaEsferReclass_{}.tif".format(ValueP)
        Morfometria = Workspace + "Morfometria_{}.tif".format(ValueP)

        # Process: Flow Direction
        arcpy.AddMessage("Process: Flow Direction")
        arcpy.gp.FlowDirection_sa(Fill_raster, FlowDir, "NORMAL", "", "D8")

        # Process: Flow Accumulation
        arcpy.AddMessage("Process: Flow Accumulation")
        arcpy.gp.FlowAccumulation_sa(FlowDir, FlowAcc, "", "FLOAT", "D8")

        # Process: Raster Calculator (Acuenca)
        arcpy.AddMessage("Process: Raster Calculator (Acuenca)")
        arcpy.gp.SingleOutputMapAlgebra_sa("(" + FlowAcc + " * float(156.25))", Acuenca)

        # Process: Reclassify (Acuenca)
        arcpy.AddMessage("Process: Reclassify (Acuenca)")
        arcpy.gp.Reclassify_sa(Acuenca, "Value", "0 1 1;1 3500 2;3500 40000 5;40000 1000000 3;1000000 839112651 1;839112651 974142273 1;974142273 1118816868 1;1118816868 1302071355 1;1302071355 2469113088 1;NODATA 1", ACuenca_Reclass, "DATA")

        # Process: Slope
        arcpy.AddMessage("Process: Slope")
        arcpy.gp.Slope_sa(Fill_raster, SlopeGrados, "DEGREE", "1", "PLANAR", "METER")

        # Process: Reclassify Slope
        arcpy.AddMessage("Process: Reclassify Slope")
        arcpy.gp.Reclassify_sa(SlopeGrados, "Value", "0 7 1;7 11 2;11 19 3;19 40 4;40 90 5", Slope_Reclass, "DATA")

        # Process: Aspect
        arcpy.AddMessage("Process: Aspect")
        arcpy.gp.Aspect_sa(Fill_raster, Aspect, "PLANAR", "METER")

        # Cálculo de varianza esférica
        arcpy.AddMessage("Calculando varianza esférica")
        # ... (incluir aquí los cálculos de X, Y, Z, XI, YI, ZI, R, RI y VarianzaEsferica) ...
        # Process: Raster Calculator (4)
        arcpy.gp.SingleOutputMapAlgebra_sa("con(" + SlopeGrados + " * float(0.0174533), 1)", Slope_Radians)
        arcpy.gp.SingleOutputMapAlgebra_sa("con(" + Aspect + " * float(0.0174533), 1)", Aspect_Radians)
        # Define variables for intermediate calculations
        X1 = Workspace + "X1_{}.tif".format(ValueP)
        X2 = Workspace + "X2_{}.tif".format(ValueP)
        X = Workspace + "X_{}.tif".format(ValueP)
        Y1 = Workspace + "Y1_{}.tif".format(ValueP)
        Y2 = Workspace + "Y2_{}.tif".format(ValueP)
        Y = Workspace + "Y_{}.tif".format(ValueP)
        Z = Workspace + "Z_{}.tif".format(ValueP)
        XI = Workspace + "XI_{}.tif".format(ValueP)
        YI = Workspace + "YI_{}.tif".format(ValueP)
        ZI = Workspace + "ZI_{}.tif".format(ValueP)
        XI2 = Workspace + "XI2_{}.tif".format(ValueP)
        YI2 = Workspace + "YI2_{}.tif".format(ValueP)
        ZI2 = Workspace + "ZI2_{}.tif".format(ValueP)
        R = Workspace + "R_{}.tif".format(ValueP)
        RI = Workspace + "RI_{}.tif".format(ValueP)

        arcpy.AddMessage("# Process: X")
        arcpy.gp.SingleOutputMapAlgebra_sa("Sin("+ Slope_Radians+")", X1)
        arcpy.gp.SingleOutputMapAlgebra_sa("Cos("+ Aspect_Radians+")",X2)
        arcpy.gp.SingleOutputMapAlgebra_sa("(" + X1 + " * " + X2 + ")", X)
        arcpy.AddMessage("# Process: Y")
        arcpy.gp.SingleOutputMapAlgebra_sa("Sin("+ Slope_Radians+")", Y1)
        arcpy.gp.SingleOutputMapAlgebra_sa("Sin("+ Aspect_Radians+")", Y2)
        arcpy.gp.SingleOutputMapAlgebra_sa("(" + Y1 + " * " + Y2 + ")", Y)
        arcpy.AddMessage("# Process: Z")
        arcpy.gp.SingleOutputMapAlgebra_sa("Cos("+Slope_Radians+")", Z)


        # Process: Focal Statistics (3)
        arcpy.AddMessage("# Process: Focal Statistics (3) ")
        arcpy.gp.FocalStatistics_sa(Z, ZI, "Rectangle 3 3 CELL", "SUM", "DATA", "90")
        # Process: Focal Statistics (2)
        arcpy.AddMessage("# Process: Focal Statistics (2) ")
        arcpy.gp.FocalStatistics_sa(Y, YI, "Rectangle 3 3 CELL", "SUM", "DATA", "90")
        # Process: Focal Statistics
        arcpy.AddMessage("# Process: Focal Statistics ")
        arcpy.gp.FocalStatistics_sa(X, XI, "Rectangle 3 3 CELL", "SUM", "DATA", "90")
        # Process: Raster Calculator (8)
        arcpy.AddMessage("# Process: Raster Calculator (FINALXXX) ")
        arcpy.gp.Square_sa(XI, XI2)
        arcpy.gp.Square_sa(YI, YI2)
        arcpy.gp.Square_sa(ZI, ZI2)
        arcpy.gp.SingleOutputMapAlgebra_sa("(" + XI2 + " + " + YI2 + " + " + ZI2 + ")", R)
        arcpy.gp.SquareRoot_sa(R, RI)
        arcpy.gp.SingleOutputMapAlgebra_sa("( 1 - ("  + RI + " / 9) * (-1) + 1)", VarianzaEsferica)

        # Process: Reclassify VarianzaEsferica
        arcpy.AddMessage("Process: Reclassify VarianzaEsferica")
        arcpy.gp.Reclassify_sa(VarianzaEsferica, "Value", "0 0.985 5;0.985 0.99 4;0.99 0.995 3;0.995 0.9975 2;0.9975 1 1", VarianzaEsferReclass, "DATA")

        # Process: Cálculo final de Morfometría
        arcpy.AddMessage("Process: Cálculo final de Morfometría")
        arcpy.gp.SingleOutputMapAlgebra_sa("(" + ACuenca_Reclass + " * 0.1) + (" + VarianzaEsferReclass + " * 0.6) + (" + Slope_Reclass + " * 0.3)", Morfometria)

        arcpy.AddMessage("Morfometría calculada para la cuenca {}".format(ValueP))
#======================================================================================

        archivos_intermedios = [
        X1, X2, X, Y1, Y2, Y, Z, XI, YI, ZI, XI2, YI2, ZI2, R, RI,
        Slope_Radians, Aspect_Radians,
        FlowDir, FlowAcc, Acuenca, SlopeGrados, Slope_Reclass, Aspect,
        VarianzaEsferica, ACuenca_Reclass, VarianzaEsferReclass,
        Workspace + "Aspect_Radians_{}.tif".format(ValueP),
        Workspace + "Slope_Radians_{}.tif".format(ValueP),
        Workspace + "X_{}.tif".format(ValueP),
        Workspace + "Y_{}.tif".format(ValueP),
        Workspace + "Z_{}.tif".format(ValueP),
        Workspace + "XI_{}.tif".format(ValueP),
        Workspace + "YI_{}.tif".format(ValueP),
        Workspace + "ZI_{}.tif".format(ValueP),
        Workspace + "XI2_{}.tif".format(ValueP),
        Workspace + "YI2_{}.tif".format(ValueP),
        Workspace + "ZI2_{}.tif".format(ValueP),
        Workspace + "R_{}.tif".format(ValueP),
        Workspace + "RI_{}.tif".format(ValueP)
        ]
        # Eliminar archivos intermedios
        arcpy.AddMessage("Eliminando archivos intermedios de morfometría...")
        for archivo in archivos_intermedios:
            if arcpy.Exists(archivo):
                try:
                    arcpy.Delete_management(archivo)
                    arcpy.AddMessage("Eliminado: {}".format(archivo))
                except Exception as e:
                    arcpy.AddWarning("No se pudo eliminar {}: {}".format(archivo, str(e)))
            else:
                arcpy.AddMessage("Archivo no encontrado: {}".format(archivo))

        arcpy.AddMessage("Proceso de eliminación de archivos intermedios completado.")

        # Conservar solo el archivo de Morfometria final
        arcpy.AddMessage("Archivo de Morfometria conservado: {}".format(Morfometria))
